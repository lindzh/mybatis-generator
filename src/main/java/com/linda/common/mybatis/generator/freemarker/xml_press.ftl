...
<mapper namespace="${mybatis.daoPackage}.${mybatis.daoClassName}">
	...
	<insert id="add${mybatis.classSimpleName}" <#if mybatis.autoGenerate>useGeneratedKeys="true" keyProperty="${mybatis.primary.property}"</#if> parameterType="${mybatis.className}">
		insert into ${mybatis.table}(<#assign idx=0><#list mybatis.columns as c>${c.column}<#assign idx=idx+1><#if (idx<mybatis.columns?size)>,</#if></#list><#if !mybatis.autoGenerate>,${mybatis.primary.column}</#if>)
		 values(<#assign idx=0><#list mybatis.columns as c>${pre}obj.${c.property}${end}<#assign idx=idx+1><#if (idx<mybatis.columns?size)>,</#if></#list><#if !mybatis.autoGenerate>,${pre}obj.${mybatis.primary.property}${end}</#if>)
	</insert>
...
<#list mybatis.uniques as u>
	...
	<select id="getBy${u.name}" resultMap="${mybatis.beanMapper}">
		select * from ${mybatis.table} where <#assign idx=0><#list u.columns as c>${c.column}=${pre}${c.property}${end}<#assign idx=idx+1><#if (idx<u.columns?size)> and </#if></#list>
	</select>
	</#if>
	...
	<delete id="deleteBy${u.name}">
		delete from ${mybatis.table} where <#assign idx=0><#list u.columns as c>${c.column}=${pre}${c.property}${end}<#assign idx=idx+1><#if (idx<u.columns?size)> and </#if></#list>
	</delete>
	</#if>
...
<#list mybatis.indexes as index>
	...
	<select id="getListBy${index.name}" resultMap="${mybatis.beanMapper}">
		...
		<#assign idx=0><#list index.columns as c>${c.column}=${pre}${c.property}${end}<#assign idx=idx+1><#if (idx<index.columns?size)> and </#if></#list>
